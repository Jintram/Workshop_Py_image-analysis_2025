...
img_path_KTR = \
    '/Users/m.wehrens/Data_notbacked/'+\
    '2025_Py-Image-workshop_KTR-example-data/raw/Composite_KTR.tif'
...
*Image taken from [biobook](https://bioimagebook.github.io/chapters/2-processing/3-thresholding/thresholding.html) (fig. 68), [Creative Commons Attribution 4.0 International License](https://creativecommons.org/licenses/by/4.0/).*
...
*"Happy cell" [image source](https://bioimagebook.github.io/chapters/2-processing/3-thresholding/thresholding.html), [Creative Commons Attribution 4.0 International License](https://creativecommons.org/licenses/by/4.0/).*
...
# Note that the offset parameter is subtracted from preliminary threshold, 
# therefor -10
...
# The Triangle method
mask_ecoli_triangle = \
    img_ecoli_inv > sk.filters.threshold_triangle(img_ecoli_inv)
...
mask_ecoli_local = \
    img_ecoli_inv > sk.filters.threshold_local(
        img_ecoli_inv, block_size=35, offset=-10
        )
...
'local neighborhood'.
Let's look at a **general ways of local image processing**, which are fundamental in image processing, 
...
- Write a function to segment a picture of nuclei, e.g. `seg_nuclei(input_img)`.
    - The function should return a labeled mask, where each label
    corresponds to the respective pixels of a single nucleus.
    - Test that it works.
- Write a function that segments a picture of bacteria, e.g. `seg_bacterium(input_img)`.
    - The function should return a labeled mask, where each label
    corresponds to the respective pixels of a single bacterium.
    - Test that it works.
